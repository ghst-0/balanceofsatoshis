...
const getFeesChart = (args, cbk) => {
  return new Promise((resolve, reject) => {
    asyncAuto({
      ...
      validate: _cbk => {
        if (!args.fs) {
          return _cbk([400, 'ExpectedFileSystemMethodsToGetFeesChart']);
        }
        ...
        if (!!args.end_date && !args.start_date) {
          return _cbk([400, 'ExpectedStartDateToRangeToEndDateForFeesChart']);
        }

        if (!isDate(args.start_date)) {
          return _cbk([400, 'ExpectedValidDateTypeForFeesChartStartDate']);
        }

        if (!moment(args.start_date).isValid()) {
          return _cbk([400, 'ExpectedValidStartDateForFeesChartEndDate']);
        }
        ...
        if (args.start_date > args.end_date) {
          return _cbk([400, 'ExpectedStartDateBeforeEndDateForFeesChart']);
        }

        if (!isDate(args.end_date)) {
          return _cbk([400, 'ExpectedValidDateFormatForFeesChartEndDate']);
        }

        if (!moment(args.end_date).isValid()) {
          return _cbk([400, 'ExpectedValidEndDateForFeesChartEndDate']);
        }
      ...
      getNode: ['viaTag', ({viaTag}, _cbk) => {
        ...
        return getNodeAlias({lnd, id: args.via}, _cbk);
      }],

      // Fees via nodes
      via: ['viaTag', ({viaTag}, _cbk) => {
        if (viaTag) {
          return _cbk(null, viaTag.nodes);
        }

        if (args.via) {
          return _cbk(null, [args.via]);
        }
      ...
      // Get forwards
      getForwards: ['start', 'end', 'via', ({start, end, via}, _cbk) => {
      ...
      forwards: ['getForwards', ({getForwards}, _cbk) => {
        ...
        return _cbk(null, forwards.reduce((sum, n) => sum.concat(n), []));
      }],

      // Total earnings
      totalEarned: ['forwards', ({forwards}, _cbk) => {
        return _cbk(null, forwards.reduce((sum, {fee}) => sum + fee, Number()));
      }],

      // Total forwarded
      totalForwarded: ['forwards', ({forwards}, _cbk) => {
      ...
      description: [
        ...
        'totalEarned',
        'totalForwarded',
        ({end, forwards, measure, start, totalEarned, totalForwarded, sum}, _cbk) =>
      {
      ...
      head: ['validate', ({}, _cbk) => {
        if (args.is_count) {
          return _cbk(null, 'Forwards count');
        }

        if (args.is_forwarded) {
          return _cbk(null, 'Forwarded amount');
        }

        return _cbk(null, 'Routing fees earned');
      }],

      // Summary title of the fees earned
      title: ['getNode', 'head', 'viaTag', ({getNode, head, viaTag}, _cbk) => {
        // Exit early when no via is specified
        if (!args.via) {
          return _cbk(null, head);
        }

        const via = viaTag || getNode;

        return _cbk(null, `${head} via ${via.alias || via.id}`);
      }],
      ...
      data: [
        ...
        'sum',
        'title',
        ({description, sum, title}, _cbk) =>
      {
        if (args.is_count) {
          return _cbk(null, {description, title, data: sum.count});
        }

        if (args.is_forwarded) {
          return _cbk(null, {description, title, data: sum.forwarded});
        }

        return _cbk(null, {description, title, data: sum.fees});
      }],
    },
